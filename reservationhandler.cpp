...
ReservationHandler::ReservationHandler(QObject *parent) : QObject(parent)
{
    connectDatabase();
    cleanOldReservations();
}
void ReservationHandler::exportToPDF(const QString &fullName, const QString &route, const QString &date, const QString &seat, const QString &phone) {
    QString filePath = QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation)
    + QDir::separator() + "Ticket_" + QDateTime::currentDateTime().toString("yyyyMMdd_hhmmss") + ".pdf";
    ...
    const QrCode qr = QrCode::encodeText(("Passenger: " + fullName + ", Seat: " + seat).toUtf8().constData(), QrCode::Ecc::LOW);
...
bool ReservationHandler::saveReservation(QString name, QString phone, QString method,
